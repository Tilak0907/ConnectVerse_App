...
function App() {
  ...
  const [keywords, setKeywords] = useState(''); // State for the keywords
  ...
  const handleUpload = () => {
    if (!file || !caption.trim()) {
      alert("Please select a file and enter a caption before uploading.");
      return;
    }
        ...
        <div className="image-gallery" style={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
          {posts.map(post => (
            ...
            >
              ...
              <p><strong>Keywords:</strong> {Array.isArray(post.keywords) ? post.keywords.join(', ') : 'No keywords'}</p>
